 ...
 */
function sellInventoryItemAt(state, inventoryIndex, data, fluctuacionMercado) {
  const item = state.inventory[inventoryIndex];
  if (!item) return { ok: false, error: 'Ese objeto ya no está en el inventario.' };
  const orderIndex = findMatchingOrderIndex(state, item.categoria);
  ...
  const price = order ? basePrice * order.mult : basePrice;
  state.money += price;
 ...
 * @returns {number}
 */
function pickVendorSaleIndex(state) {
...
export function stallVendorTick(state, data, now = Date.now(), random = Math.random) {
  ...
  const idx = pickVendorSaleIndex(state);
  if (idx === -1) return;
  sellInventoryItem(state, idx, data, now, random);
}
...
export function applyOfflineStallSales(state, data, segundosEfectivos) {
  ...
  for (let i = 0; i < count; i++) {
    const idx = pickVendorSaleIndex(state);
    if (idx === -1) break;
    const result = sellInventoryItemAt(state, idx, data, 1);
